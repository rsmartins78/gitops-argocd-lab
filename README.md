...
- Helm
- Minikube
- ArgoCD CLI (for advanced management)
...
- Run the deploy script to build and push the Docker image, and update the Helm values:
  ...
  ./deploy.sh
  ```
  This will check if the image was built (see the Github Actions workflow), and apply the ArgoCD ApplicationSet.
...
- If you didn't store the password after running `./setup.sh`, retrieve it again running `argocd admin initial-password -n argocd`.
